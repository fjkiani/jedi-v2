          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-12">
              {useCasesData.length === 0 ? (
                <p className={`md:col-span-2 text-center ${isDarkMode ? 'text-n-4' : 'text-n-5'} py-10`}>No use cases available at the moment.</p>
              ) : (
                useCasesData.map((useCase) => (
                  ...
                  >
                    ...
                    >
                      <div className={`rounded-2xl overflow-hidden h-full w-full p-8 lg:p-10 flex flex-col transition-all 
                      ...
                      `}>
                        <div className="mb-6">
                          <span className={`text-xs uppercase tracking-wider ${isDarkMode ? 'text-n-3' : 'text-n-5'} mb-2 block`}>
                            {useCase.industry?.name || 'Industry'}
                          </span>
                          <h3 className={`h3 mb-2 ${isDarkMode ? 'text-n-1' : 'text-n-8'}`}>{useCase.title}</h3>
                        </div>
                        ...
                        {useCase.technologies && useCase.technologies.length > 0 && (
                          <div className="mb-8 flex-grow">
                            <h4 className={`text-lg font-semibold ${isDarkMode ? 'text-color-1' : 'text-primary-1'} mb-2 flex items-center`}>
                              <FiCpu className={`mr-2 ${isDarkMode ? 'opacity-80' : 'opacity-100'}`} size={18}/> Technologies Used
                            </h4>
                            <div className="flex flex-wrap gap-2">
                              {useCase.technologies.map((tech) => (
                                ...
                                >
                                  {tech.icon && (
                                    <img src={tech.icon} alt={tech.name} className="w-4 h-4 mr-1.5 object-contain" />
                                  )}
