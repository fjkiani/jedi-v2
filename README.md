...
# NOTE: Assumes ComponentCreateInput, FlowStepCreateInput, ArchitectureCreateInput,
# UseCaseCreateInput are the correct input types generated by Hygraph.

mutation AddUseCaseToTechnology(
  # --- Identify Target Technology ---
  $technologySlug: String!

  # --- Identify Target Industry (to connect) ---
  $industrySlug: String!
  ...
  $useCaseCapabilities: [String!]

  # --- Architecture Data ---
  $architectureDescription: String!

  # --- Components Data (Array of objects) ---
  # Define input type matching Component fields
  # type ComponentCreateInput { name: String!, description: String!, technologies: [String!], details: String!, explanation: String! }
  $componentsToCreate: [ComponentCreateInput!]

  # --- FlowSteps Data (Array of objects) ---
  # Define input type matching FlowStep fields
  # type FlowStepCreateInput { step: String!, description: String!, details: String! }
  $flowStepsToCreate: [FlowStepCreateInput!]

) {
  updateTechnology(
    where: { slug: $technologySlug }
    data: {
      # Use the relation field from Technology to UseCase
      relatedUseCases: {
        create: [
          {
            # --- UseCase Fields ---
            ...
            queries: { set: $useCaseQueries }
            capabilities: { set: $useCaseCapabilities }

            # Connect to an existing Industry
            industry: {
              connect: { slug: $industrySlug }
              # Alternatively, to create a new Industry:
              # create: { name: "New Industry Name", slug: "new-industry-slug", sections: ["Section A"] }
            }

            # Create the nested Architecture
            architecture: {
              create: {
                # --- Architecture Fields ---
                ...
                # Create nested FlowSteps
                flow: { create: $flowStepsToCreate }
              }
            }
          }
          # Add more UseCase objects here to create multiple in one go
        ]
      }
    }
  ) {
    ...
    slug
    name
    relatedUseCases(last: 1) { # Get the newly created UseCase
      id
      title
      industry {
        id
